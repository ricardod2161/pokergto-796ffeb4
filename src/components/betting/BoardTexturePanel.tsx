...
export function BoardTexturePanel({ texture, isOESD }: BoardTexturePanelProps) {
  ...
  const getStraightDrawBadgeColor = () => {
    ...
    const effectiveIsOESD = isOESD !== undefined ? isOESD : texture.straightDraw === "oesd";
    return effectiveIsOESD ? "text-success font-semibold" : "text-warning font-semibold";
  };

  const properties = [
    {
      label: "Flush Draws",
      value: flushDrawLabels[texture.flushDraw],
      active: texture.flushDraw !== "none",
      colorClass: ""
    },
    {
      label: "Straight Draws",
      value: getStraightDrawLabel(),
      active: texture.straightDraw !== "none",
      colorClass: getStraightDrawBadgeColor()
    },
    {
      label: "Pareado",
      value: texture.paired ? "Sim" : "Não",
      active: texture.paired,
      colorClass: ""
    },
    {
      label: "Trips",
      value: texture.trips ? "Sim" : "Não",
      active: texture.trips,
      colorClass: ""
    },
    {
      label: "Conectado",
      value: texture.connected ? "Sim" : "Não",
      active: texture.connected,
      colorClass: ""
    },
    {
      label: "Rainbow",
      value: texture.rainbow ? "Sim" : texture.monotone ? "Monotone" : "Não",
      active: !texture.rainbow,
      colorClass: ""
    },
    {
      label: "High Card",
      value: texture.highCard,
      active: true,
      colorClass: ""
    },
    // Wheel possible — only shown when relevant
    ...(texture.wheelPossible ? [{
      label: "Wheel Possível",
      value: "A-2-3-4-5",
      active: true,
